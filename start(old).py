...
def func_Dz(Dw,dz,Dz):
    return (Dw - ((4*(Dz**2) - (math.pi * (dz**2)))/(4*Dz + math.pi*dz)))

#klasa wczytujaca pliki csv
#bierze adres pliku 
#linie(kolumny czyli A,B,C...) liczymy od 0 a rzedy(czyli 1,2,3,4...) od 1
#test klasy: print(csv_load("wyciep projekt 2 warianty.csv", 1, 1))
def csv_load(adress, number_line, number_row):
#to wczytuje nam plik i to "r" mowi w jaki format to jest
    with open(adress, 'r') as csvfile:
        ...
        for line in spamreader:
            i +=1
            if i == number_line:
                return line[number_row]

#klasa zapisujaca plik csv
#bierze adress pliku a jak go nie ma to go twozy
def csv_save(adress, number_of_wariant, tablica):
...
#klasa ta jest szablonem który zaczyna projekt do wyciepu zad1 lam
#bierze adres pliku oraz numer wariantu czuli numer rzedu od 1 do ilus tam
#zwraca arrey [re, m, Q, alfap, Tscian, Tpsr, alfaw, Qk, Tout_olb] czyli wszystko co trzeba do tabeli 
def wyciep_projekt_2_zad1(adress, wariant, re):
    #wczytanie danych 
    Tw = float(csv_load(adress, wariant, 2))#temperatura wlotowa
    Tp = float(csv_load(adress, wariant, 3))#temperatura powietrza
    L = float(csv_load(adress, wariant, 4))#długość rurociagu
    dw = float(csv_load(adress, wariant, 5))#srednica wewnetrzna 
    dz = dw + 0.0008 #srednica zewnetrza 
    Tout_zal = float(360) #zalozona temepratura wylotowa
    g = float(9.81)
...
def wyciep_projekt_2_zad2(adress, wariant):
    #wczytanie danych 
    rodzaj_wymienika = csv_load(adress, wariant, 6)#liczby 1 dla kwadratowego 2 dla okraglego
    T1 = float(csv_load(adress, wariant, 7))#temperatura 1
    T2 = float(csv_load(adress, wariant, 8))#temperatura 2
    m = float(csv_load(adress, wariant, 9))#strumien masy
    rodzaj_przepływu = csv_load(adress, wariant, 10)#litery p albo w 
    t1 = float(csv_load(adress, wariant, 11))
    t2 = float(csv_load(adress, wariant, 12))
    mz = (m*1000)/(3600*24)
    Tsrz = (T1+T2)/2
    Tsrg = (t1+t2)/2
    ug = 1.8
...
def docx_save(imie, nazwisko, numer_warantu, tablica1, tablica2):
    ...
    document.add_paragraph('Tw=%s[k]'%(csv_load("wyciep projekt 2 warianty.csv", numer_warantu, 2)))
    document.add_paragraph('Tp=%s[k]'%(csv_load("wyciep projekt 2 warianty.csv", numer_warantu, 3)))
    document.add_paragraph('L=%s[m]'%(csv_load("wyciep projekt 2 warianty.csv", numer_warantu, 4)))
    document.add_paragraph('dw=%s[m]'%(csv_load("wyciep projekt 2 warianty.csv", numer_warantu, 5)))
    document.add_paragraph('dz=%s[m]'%(float(csv_load("wyciep projekt 2 warianty.csv", numer_warantu, 5))+0.008))
...
def start():
    ...
    #to liczy ile jest osób w wariantach 
    with open("wyciep projekt 2 warianty.csv", 'r') as csvfile:
        spamreader = csv.reader(csvfile, delimiter=';')
        number_of_people = 1
        for line in spamreader:
            number_of_people +=1
    ...
    tablica1 = {}
    tablica2 = {}
    for people in range(1,number_of_people):
        print(people)
        #do zadania 1
        tablica1[people] = []
        for Re in range(100,100001,100):
            tablica1[people].append(wyciep_projekt_2_zad1("wyciep projekt 2 warianty.csv", people, Re))
            
        csv_save("exele/%d.csv"%people, 1, tablica1[people])
        #do zadania 2
        tablica2[people] = []
        tablica2[people].append(wyciep_projekt_2_zad2("wyciep projekt 2 warianty.csv", people))
        #twozenie wykresów 
        graphing1(tablica1[people], "%s.1"%people)
        #do worda 
        docx_save(csv_load("wyciep projekt 2 warianty.csv", people, 1), csv_load("wyciep projekt 2 warianty.csv", people, 0),1,tablica1[people],tablica2[people])
        print("zakończono osobe: ", people)
        
    print("zakończony przetrwarzanie wszystkich waraintów !!! KJN i HUJ")
    print("trwało to %s sekund"%(round(time.time()-start_time)))
    print("Na wariant schodzi mi %s"%(round((time.time()-start_time)/number_of_people)))
